...
public class OpenMpApiCodeGen : IIncrementalGenerator
{
    ...
    private static MethodContext CreateContext(StructDecl node, MethodDeclarationSyntax methodDeclaration, IMethodSymbol methodSymbol)
    {
        var ctx = new MethodContext
        {
            ...
            IsUnsafe = methodDeclaration.HasModifier(SyntaxKind.UnsafeKeyword),
        ...
        ctx.ProxyMethodName = methodSymbol.GetAttributes(Constants.FunctionAttributeFQN).FirstOrDefault()?.ConstructorArguments[0].Value is string functionName
